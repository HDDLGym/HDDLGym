...
'''
from hddl_utils import enumerate_list, enumerate_index_to_binary_matrix, extract_object_list, check_task_method_action
...
### HELPER FUNCTIONS:
def pick_policy(agent_policy_dict, env_dictionary, current_hierarchy, preferences=None):
    '''
    Help choose the write policy in the agent_policy_dict to update the hierarchy
    inputs: 
    - agent_policy_dict: dictionary of policies of agent {type: policy}, e.g. {'all operators': policy_oper, 'actions': policy_action}
        Notes that the keys for agent_policy_dict are 'all operators', 'actions', 'methods', and 'subtasks'
    - current_hierarchy: a list of current hierarchy of agent (to help determine which policy to use)
    - preferences: (default: None) a string of which type of policy is prefered, if more than one policy are appropriate for the current_hierarchy, use preferences to determine
    '''
    map_rules = dict({
        "method"    : ["all operators", "subtasks"],
        "task"  : ["all operators", "methods"],
        "action"    : ["all operators", "actions"]
    })
    policy_types = list(agent_policy_dict.keys())
    if preferences == "actions" and "actions" in policy_types:
        # This is often for the non-hierarchical policy, only consider policy for primitive actions
        return agent_policy_dict['actions']
    if 'all operators' in policy_types and preferences not in policy_types:
        #always use 'all operators' policy if preferences is not specified or suitable
        return agent_policy_dict['all operators']
    elif preferences in policy_types and len(current_hierarchy)>0:
        # check if the preferences is suitable:
        type_of_last_element_hierarchy = check_task_method_action(env_dictionary, current_hierarchy[-1])
        suitable_types = map_rules[type_of_last_element_hierarchy]
        if preferences in suitable_types:
            return agent_policy_dict[preferences]
        elif "all operators" in policy_types:
            return agent_policy_dict["all operators"]
        else:
            # No suitable policy from the agent_policy_dict
            return None 
    else:
        return None
...
def get_prob_from_prob_list(operator_str, prob_list, env_dictionary,device=False):
...
def get_grounded_prob_list_from_policy_output(grounded_oper_list, prob_list, env_dictionary, device=False):
  ...
  - env_dictionary: dictionary of environment's elements
  outputs:
  - a torch.tensor of grounded probabilities list (each element is a float number in range [0,1])
  '''
  if not device:
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def get_probability_of_valid_operator_combinations(valid_operator_combination_list, policy_list, main_agent_index, current_state, env_dictionary, agents, device=False):
    ...
    main_policy_dict = policy_list[main_agent_index]
    main_policy = pick_policy(main_policy_dict, env_dictionary, agents[main_agent_index].task_method_hierarchy)
...
def enumerate_state(env, opt):
    ...
    - opt: parameters
    outputs:
    - s_num: a one-hot array for the current state - a torch tensor
    '''
...
from utils import run_planner_and_get_action_dict
def evaluate_policy(env, policy, turns = 3, opt=None, debug=False, deterministic=True, return_plan_time=False, return_success_rate=True):
        if debug:
        ...
        if opt is None:
            ...
            opt.debug = False
            opt.gamma = 0.99
            opt.use_central_planner = False
        
        policy_list = policy_list=[{'all operators':policy}]*env.num_agents
        for j in range(turns):
                env.reset(policy_list=[{'all operators':policy}]*len(env.agents))
                ...
                while not done:
                    ...
                    plan_time_turn += time.time() - start_plan_time
                    if debug:
                        print(f'Action_dict at step {step} is\n {action_dict}\n')
                    s_next, r, dw, tr, info = env.step(action_dict) # dw: dead&win; tr: truncated
                    
                    done = (dw or tr)
                    turn_score += r*(opt.gamma**step)
                    step += 1
                    ...
                    if done:
                        plan_time_list.append(plan_time_turn)
                        if len(info) == len(env.env_dictionary['htn goal'].tasks):
                            success_count += 1
                            success_steps_list.append(step)
                    if done and len(info) == len(env.env_dictionary['htn goal'].tasks) and debug: #goal reached
        ...
        if return_success_rate and return_plan_time:
            print("Success rate is: {}%".format(success_count/turns*100))
            print("Average steps to reach goal is: {}".format(np.mean(success_steps_list)))
            return int(total_scores/turns), sum(plan_time_list)/turns, success_count/turns*100, np.mean(success_steps_list)
        if return_plan_time:
            return int(total_scores/turns), sum(plan_time_list)/turns
...
### DEFINE RL MODEL: PPO for discrete problems
...
class PPO_discrete():
    def __init__(self, **kwargs):
        ...
        self.s_hoder = torch.zeros((self.T_horizon, self.state_dim), device=self.dvc, dtype=torch.float32)
        self.a_hoder = torch.zeros((self.T_horizon, self.action_dim), device=self.dvc, dtype=torch.int64)
        self.r_hoder = torch.zeros((self.T_horizon, 1), dtype=torch.float32, device=self.dvc)
        self.s_next_hoder = torch.zeros((self.T_horizon, self.state_dim), dtype=torch.float32, device=self.dvc)
        self.prob_a_hoder = torch.zeros((self.T_horizon, 1), dtype=torch.float32, device=self.dvc)
        self.done_hoder = torch.zeros((self.T_horizon, 1), dtype=torch.bool, device=self.dvc)
        self.dw_hoder = torch.zeros((self.T_horizon, 1), dtype=torch.bool, device=self.dvc)
    ...
    def train(self):
        ...
        if self.monitor_training:
            print("device in training: ", self.dvc)

        self.entropy_coef *= self.entropy_coef_decay #exploring decay
        '''Prepare PyTorch data from Numpy data'''
        if isinstance(self.s_hoder, np.ndarray):
            s = torch.from_numpy(self.s_hoder).to(self.dvc)
        else:
            s = self.s_hoder.to(self.dvc)
        if isinstance(self.a_hoder, np.ndarray):
            a = torch.from_numpy(self.a_hoder).to(self.dvc)
        else:
            a = self.a_hoder.to(self.dvc)
        if isinstance(self.r_hoder, np.ndarray):
            r = torch.from_numpy(self.r_hoder).to(self.dvc)
        else:
            r = self.r_hoder.to(self.dvc)
        if isinstance(self.s_next_hoder, np.ndarray):
            s_next = torch.from_numpy(self.s_next_hoder).to(self.dvc)
        else:
            s_next = self.s_next_hoder.to(self.dvc)
        if isinstance(self.prob_a_hoder, np.ndarray):
            old_prob_a = torch.from_numpy(self.prob_a_hoder).to(self.dvc)
        else:
            old_prob_a = self.prob_a_hoder.to(self.dvc)
        if isinstance(self.done_hoder, np.ndarray):
            done = torch.from_numpy(self.done_hoder).to(self.dvc)
        else:
            done = self.done_hoder.to(self.dvc)
        if isinstance(self.dw_hoder, np.ndarray):
            dw = torch.from_numpy(self.dw_hoder).to(self.dvc)
        else:
            dw = self.dw_hoder.to(self.dvc)
        ...
        for _ in range(self.K_epochs):
            ...
            for i in range(optim_iter_num):
                ...
                # # Probability of action a includes lifted action and objects
                # print("prob*a[index]:",prob*a[index])                
                ...
                prob_ = prob*a[index]
                mask = prob_ > 0
                log_tensor = torch.zeros_like(prob_)
                log_tensor[mask] = torch.log(prob_[mask]) #apply log to non-zero elements
                log_prob_a = log_tensor.sum(dim=1, keepdim=True) / mask.sum(dim=1, keepdim=True).clamp(min=1)
                prob_a = torch.exp(log_prob_a)
                log_old_prob_a = torch.log(old_prob_a[index] + 1e-20)
    ...
    def save(self, episode):
        ...
        torch.save(self.critic.state_dict(), str(script_dir / "model/{}/ppo_critic{}.pth").format(self.problem_name,episode))
        torch.save(self.actor.state_dict(), str(script_dir / "model/{}/ppo_actor{}.pth").format(self.problem_name,episode))

    def load(self, episode):
        script_dir = Path(os.path.dirname(os.path.abspath(__file__)))
        self.critic.load_state_dict(torch.load(str(script_dir / "model/{}/ppo_critic{}.pth").format(self.problem_name,episode),map_location=self.dvc))
        self.actor.load_state_dict(torch.load(str(script_dir / "model/{}/ppo_actor{}.pth").format(self.problem_name,episode), map_location=self.dvc))
        print("Loaded model from episode {}".format(episode))