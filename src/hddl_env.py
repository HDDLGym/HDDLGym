# MODIFY IF NEEDED: Change learning_methods to your own learning methods file:
from learning_methods import get_observation_space, get_action_space

from hddl_utils import extract_object_list, extract_object_list, Action
from hddl_parser import main_parser
from central_planner_utils import check_valid_combination_operator
...
class WorldInfo():
    def __init__(self, num_agents, current_state, env_dictionary):
        self.env_dictionary = env_dictionary
        self.current_state = current_state
        self.num_agents = num_agents

class HDDLEnv(gym.Env):
    def __init__(self, domain_file, problem_file, policy_list = [],max_step=50):
    ...
    def initialize_agents(self, policy_list, htn_goal=None):
      ...
      # assign value for self.num_agents:
      self.num_agents = len(self.agents)
      # set the belief_other_agents of each agent to be the rest of the agents:
    ...
    def extract_world_info(self):
        """Extracts the world info as a WorldInfo object."""
        return WorldInfo(
            num_agents=self.num_agents,
            current_state=copy.deepcopy(self.current_state),
            env_dictionary=copy.deepcopy(self.env_dictionary)
        )
    ...
    def get_reward(self, costs, goals_reached, no_conflict):
        ...
        '''
        reward = 0
        reward -= sum(costs) #0 #simple case
        if not no_conflict:
            # if conflict:
            reward -= 0#10
        if goals_reached:
