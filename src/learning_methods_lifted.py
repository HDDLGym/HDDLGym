...
Its input is the one-hot code array of the dynamic grounded predicates, lifted operators used in the previous step.
Its output is the probability of the lifted operators. 
From this output, the probability of the grounded operators can be calculated by combine lifted operators.
...
def get_prob_from_prob_list(operator_str, prob_list, env_dictionary,device='cpu'):
  ...
  - prob_list: a tensor, listing prob of lifted operators
...
def get_observation_one_hot_vector(current_state, all_operators, env_dictionary, device=False):
    ...
    observation = torch.cat([state_num_tensor, lifted_operator_matrix])
...
def get_probability_of_valid_operator_combinations(valid_operator_combination_list, policy_list, main_agent_index, current_state, env_dictionary, agents, device=False):
    ...
    if main_policy is None:
        probability_list_of_combinations = [1/(len(valid_operator_combination_list)+1e-20)]*len(valid_operator_combination_list)
        return torch.tensor(probability_list_of_combinations, device=device)
...
def enumerate_action(env, opt):
    ...
    '''
    a_num = torch.zeros((len(env.env_dictionary['lifted operators list']),), device=opt.dvc, dtype=torch.float32)
    for agent_ in env.agents:
    ...
    #                       a_num_a_objects.clone().detach()], dim=0).to(opt.dvc, dtype=torch.float32)
      a_num += a_num_a_lifted.clone().detach()
    ...
    if opt.use_central_planner:
        ...
        for i,ag in enumerate(env.agents):
            ...
            ag.decentralize_planner_agent(env, belief_other_agents = belief_other_agents,agent_policy = policy_list[i],\
              ...
              time_limit=opt.planner_time_limit, debug=debug)
...
def get_logprob(s_num, policy, env, opt):
    ...
    for agent in env.agents:
        if len(agent.prob_hierarchy)>0:
            ...
            logprob_a += torch.mean(torch.log(prob_oper + 1e-20))
...
def evaluate_policy(env, policy, turns = 3, opt=None, debug=False, deterministic = True):
        '''
        Evaluating policy by running thru the env until done or exceed maximum number of steps.
        inputs:
        - env: an HDDLEnv instance
        - policy: either None or a RL policy that has selection_action method
        - turns: number of episode to run
        - opt: parameters
        - debug: boolean, whether to print out results of each step and hierarchy record
        - deterministic: boolean, whether to use policy in deterministic way
        outputs: 
        - scores: average of total reward of each turn
        '''
        print("Start evaluating policy {} ...".format(policy))
        total_scores = 0
        for j in range(turns):
                ...
                while not done:
                    action_dict, env, hierarchies = run_planner_and_get_action_dict(policy_list, env, opt, deterministic=deterministic, debug=opt.debug)
                    if debug:
                    ...
                    if debug and done and len(info) == len(env.env_dictionary['htn goal'].tasks): #goal reached
