...
def remove_comments_from_hddl(input_file, output_file):
    """
...
def check_hddl_format(file_path):
    ...
    '''
    errors = []
    temp_file = "HDDL_files/temp_file.hddl"
    remove_comments_from_hddl(file_path, temp_file)
    type_hierarchy = parse_types(temp_file)
    if type_hierarchy:
        if not is_agent_in_hierarchy(type_hierarchy):
            errors.append("No 'agent' in type!")
    ...
    # Check if none-action is defined as required:
    if not check_none_action_defined_correctly(temp_file, with_none_name=True):
        errors.append("The none action is missing or incorrectly formatted.")
        errors.append(check_none_action_defined_correctly(temp_file, with_none_name=False))

    if not errors:
        print("HDDL file format is correct.")
    else:
        print("Errors found in HDDL file:")
        for error in errors:
            print(f"- {error}")
...
def parse_types(file_path, debug=False):
    debug_message = "Parsing types from the HDDL file.\n"
    with open(file_path, 'r') as file:
        content = file.read()

    # Extract the :types section using regex
    types_section = re.search(r'\(:types\s+(.*?)(?=\)|\(:|$)', content, re.DOTALL)
    debug_message += f"Found types section without close paren: {types_section.group(1) if types_section else 'None'}\n"
    
    
    if not types_section:
        print("No ':types' section found in the HDDL file.")
        return None
    
    type_lines = types_section.group(1).splitlines()
    debug_message += f"Type lines extracted: {type_lines}\n"
    ...
    for line in type_lines:
        ...
        if not line or line.startswith(';'):
            continue
        if ' - ' in line:
            # Split the line into subtypes and supertype
            parts = line.split(' - ')
            if len(parts) == 2:
                subtype = parts[0].strip()
                supertype = parts[1].strip()
                subtype_list = subtype.split() # in case of multiple subtypes
                for subtype in subtype_list:
                    type_hierarchy[supertype].append(subtype)
            else:
                print(f"Malformed type line: {line}")
        else:
            # If no supertype is specified, treat it as a root type
            subtype = line.strip()
            type_hierarchy['root'].append(subtype)
    ...
    if debug:
        return type_hierarchy, debug_message
    return type_hierarchy

def is_agent_in_hierarchy(type_hierarchy, target='agent', debug=False):
...
def find_actions_without_agent(file_path, agent_type='agent'):
    ...
    print("Agent-related types:", agent_related_types)
...
def main_modify(input_file, output_file):
    ...
    # 1. remove comments
    remove_comments_from_hddl(input_file, output_file)
