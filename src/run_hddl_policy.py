...
def run_hddl_model(opt):
    ...
    # opt.max_e_steps = min(opt.max_e_steps, eval_env._max_episode_steps)
    
    policy = PPO_discrete(**vars(opt)).to(opt.dvc)
    policy.load(opt.Model)
    policy_dict = {"all_operators":policy}

    score, action_array_list, hierarchies_array_list, belief_hierarchy_0_array, belief_hierarchy_1_array = get_policy_output(eval_env, policy_dict, turns=1, opt=opt)  # Run the evaluation for 10 episodes
...
def get_policy_output(env, policy, turns = 3, opt=None):
        ...
        for j in range(turns):
                ...
                while not done:
                    ...
                    if len(env.agents) == 2:
                        belief_hierarchy_0 = env.agents[0].belief_other_agents[1].task_method_hierarchy
