...
def random_select_problem_file(prob_dir):
    ''' Random select a problem file from the folder prob_dir
    '''
    # List all .hddl files (non-recursive)
    hddl_files = [
        os.path.join(prob_dir, f)
        for f in os.listdir(prob_dir)
        if f.lower().endswith(".hddl") and os.path.isfile(os.path.join(prob_dir, f))
    ]
    
    if not hddl_files:
        raise FileNotFoundError(f"No .hddl files found in folder: {prob_dir}")
    
    # Randomly pick one
    selected = random.choice(hddl_files)
    print(f"Randomly selected HDDL file: {selected}")
    return selected
...
def main_parser(domain_file, prob_file, debug=False):
    ...
    if os.path.isdir(prob_file):
        selected_prob = random_select_problem_file(prob_file)
        prob_hddl = read_hddl(selected_prob)
    elif os.path.isfile(prob_file) and prob_file.lower().endswith(".hddl"):
        prob_hddl = read_hddl(prob_file)
    else:
        raise ValueError(f"The provided file is not an .hddl file or a folder: {prob_file}")
