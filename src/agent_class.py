...
from hddl_utils import extract_object_list, check_subtask_with_world_state, check_task_method_action
...
class Agent():
  ...
  def __init__(self, agent_name, agent_types, htn_goal = None, policy=None, belief_flag=False):
    ...
    inputs:
    - agent_name: a string of agent name, eg. 'truck-0'
    - agent_types: list of string of agent types, eg. ['vehicle', 'locatable']
    - htn_goal: a HTNGoal object, default to None, usually set to be htn_goal object from environment_dictionary
    - policy: a policy object, default to None, usually set to be HDDLEnv.policy
    - belief_flag: boolean, default to False, whether the agent is a belief agent or not
    '''
    ...
    self.current_valid_operators = set()
  ...
  def get_agent_observation(self, world_current_state, env_dict, debug=False):
    ...
    for agent_ in [self]+self.belief_other_agents:
      all_operators += self.prev_task_method_hierarchy
      if self.htn_goal != None:
        all_operators += self.htn_goal.pending_tasks
        all_operators += self.htn_goal.remaining_tasks
    all_operators = set(all_operators)
    all_operators = list(all_operators)
    observation_num = get_observation_one_hot_vector(world_current_state, all_operators, env_dict)
  ...
  def decentralize_planner_agent(self, world_info, belief_other_agents = [], agent_policy = None, other_agent_policy_list = [], debug=False, deterministic=False, device=False, time_limit=30):
    '''This function choose the next operators for the hierarchy, until it reachs action
    inputs:
    - world_info: mainly for world_info.num_agents, world_info.state, world_info.env
    ...
    if len(belief_other_agents) == world_info.num_agents - 1:
    ...
    list_agents = centralized_planner(world_info, main_agent_index = 0,all_agents = list_agents, all_policies = policy_list, debug=debug,\
