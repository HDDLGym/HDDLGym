from hddl_utils import extract_object_list, Task, Method, check_subtask_with_world_state, check_task_method_action
...
def choose_method_policy(env_dictionary, state, task, agent, method_policy = None, print_valid_list = False, debug=False, collaborative=False, collab_agents = []):
  ...
  for i in range(len(grounded_methods_list)):
    for j in range(len(grounded_methods_list[i])):
      if grounded_methods_list[i][j] in agent.current_valid_operators:
      ...
      if lifted_methods_list[i].precondition.check_precondition(state, var_obj_dict, debug=debug) and grounded_methods_list[i][j] not in agent.infeasible_task_method:
        valid_grounded_methods.append(grounded_methods_list[i][j])
        agent.current_valid_operators.add(grounded_methods_list[i][j])
  ...
  if print_valid_list:
    ...
    return valid_grounded_methods, agent.current_valid_operators
  ...
  if len(valid_grounded_methods) == 0:
    ...
    return None, 0, agent.current_valid_operators
  if method_policy == None:
    ...
    prob_method = probability_list_of_valid_methods[valid_grounded_methods.index(chosen_method)]

  #5. return chosen method:
  if debug:
    print("chosen method for task {} is {} with prob {}".format(task, chosen_method,prob_method))
  return chosen_method, prob_method, agent.current_valid_operators
...
def choose_subtask_policy(env_dictionary, state, method_str, agent, completed_subtasks= [], subtask_policy=None, print_valid_list = False, collaborative=False, collab_agents=[], print_empty_by_others=False, debug=False):
    ...
    for action in env_dictionary['lifted actions list']:
      if action.name == sub_name:
        ...
        if action.precondition.check_precondition(state,var_object_dict,debug=debug) and sub not in agent.completed_tasks_agent:
          ...
          agent.current_valid_operators.add(sub)
        break
    if not sub_is_action:
      for task in env_dictionary['lifted tasks list']:
        if task.name == sub_name:
          ...
          # Check if subtask is valid by checking if any method related to the task is valid:
          _m_list, current_valid_operators = choose_method_policy(env_dictionary, state, sub, agent, method_policy=None, print_valid_list=True, debug=False)
          if debug:
          ...
          if len(_m_list) >0:# and _m not in agent.infeasible_task_method and sub not in agent.completed_tasks_agent:
            ...
            valid_subtasks_str.append(sub)
            agent.current_valid_operators.update(current_valid_operators)
          # else:
  ...
  if print_valid_list:
    if print_empty_by_others:
      ...
      return after_ordering_other_agent, empty_by_others, agent.current_valid_operators
    return after_ordering_other_agent, agent.current_valid_operators

  # 3. choose subtask from the valid subtask list
  if len(after_ordering_subtasks_str) == 0:
    agent.infeasible_task_method.append(method_str)
    if print_empty_by_others:
      return None, 0, empty_by_others, agent.current_valid_operators
    return None, 0, agent.current_valid_operators
  elif subtask_policy == None:
    if print_empty_by_others:
      return random.choice(after_ordering_other_agent), 1/len(after_ordering_other_agent), empty_by_others, agent.current_valid_operators
    return random.choice(after_ordering_other_agent), 1/len(after_ordering_other_agent), agent.current_valid_operators
  else:
    ...
    prob_s = probability_list_of_valid_subtasks[after_ordering_other_agent.index(chosen_subtask)]
    if print_empty_by_others:
      return chosen_subtask, prob_s, empty_by_others, agent.current_valid_operators
    return chosen_subtask, prob_s, agent.current_valid_operators


def generate_valid_operators(world, agent, print_empty_by_others = False, non_hierarchy = False, debug=False):
  ...
  valid_op_list = []
  empty_by_others = False
  # For non-hierarchy planning
  if non_hierarchy: 
    valid_op_list = get_grounded_valid_actions(world.env_dictionary, world.current_state, agent)
    if 'none '+agent.name not in valid_op_list:
      valid_op_list.append('none '+agent.name)
    return valid_op_list
  
  # Now we are in the hierarchy planning
  if len(agent.task_method_hierarchy) == 0:
    ...
    for t in agent.htn_goal.remaining_tasks: 
      if t not in agent.infeasible_task_method:
        valid_op_list.append(t)
    if len(valid_op_list) == 0:
      # No more goal tasks to assign:
      # Append any valid actions of the agent
      valid_op_list = get_grounded_valid_actions(world.env_dictionary, world.current_state, agent)
    ...
    valid_op_list, current_valid_operators = choose_method_policy(world.env_dictionary,\
                                        world.current_state, \
                                        agent.task_method_hierarchy[-1],\
                                        agent, method_policy=agent.agent_policy, \
                                        print_valid_list = True,\
    ...
    valid_op_list, empty_by_others, current_valid_operators = choose_subtask_policy(world.env_dictionary,\
                                                          world.current_state, \
                                                          method_str=agent.task_method_hierarchy[-1],\
                                                          agent=agent, subtask_policy = agent.agent_policy,\
                                                          print_valid_list=True, \
                                                          collaborative=collaborative, \
                                                          collab_agents = collab_agents, \
                                                          print_empty_by_others = print_empty_by_others, \
                                                          debug=False)
    agent.current_valid_operators.update(current_valid_operators)
    ...
    if debug:
      print("DEBUG: generate_valid_operators: agent {} task_method_hierarchy[-1] {} is not task or method".format(agent.name,\
                                                                                                                agent.task_method_hierarchy[-1]))
    valid_op_list = [agent.task_method_hierarchy[-1]]
  if print_empty_by_others:
    return valid_op_list, empty_by_others, agent.current_valid_operators
  return valid_op_list, agent.current_valid_operators
...
def get_grounded_valid_actions(env_dictionary, state, agent):
  '''This function is to get a list of grounded valid actions for the agent
  inputs:
  - env_dictionary: a dictionary of environment
  - state: list of predicates
  - agent: an Agent instance
  outputs:
  - grounded_valid_actions: list of grounded valid actions that includes agent.name
  '''
  grounded_valid_actions = []
  for lifted_action in env_dictionary['lifted actions list']:
    grounded_actions = lifted_action.generate_grounded_actions(env_dictionary['type object dict'])
    for action in grounded_actions:
      # check if the action is related to the agent:
      if agent.name not in action:
        continue
      # Now, check if the action is valid: (by checking precondition with state)
      action_name, action_object = extract_object_list(action)
      var_object_dict = lifted_action.translate_objects_list_to_dict(action_object)
      if lifted_action.precondition.check_precondition(state, var_object_dict):
        grounded_valid_actions.append(action)

  return grounded_valid_actions
