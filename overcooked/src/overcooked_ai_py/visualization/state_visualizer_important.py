...
class StateVisualizer:
    ...
    def render_state(self, state, grid, hud_data=None, action_probs=None):
        ...
        # print(f'self.is_rendering_hud {self.is_rendering_hud} hud_data {hud_data}')
    ...
    def _draw_divider_around_tile(self, surface, tile_position):
        ...
        fill_color = (205, 133, 63)  # Light brown color (RGB)
        divider_color = (0, 0, 0)    # Black color for the divider
...
#################################################################################
    ...
    # def get_markable_tiles(self, grid):
    #     markable_tiles_set = set()
    #     for row in range(len(grid)):
    #         for col in range(len(grid[row])):
    #             if grid[row][col] == COUNTER and self.is_adjacent_to_floor(row, col, grid):
    #                 markable_tiles_set.add((row, col))
    #     return markable_tiles_set

    # def is_adjacent_to_floor(self, row, col, grid):
    ...
    # def mark_tiles(self, grid_surface, grid):
    #     self.markable_tiles = self.get_markable_tiles(grid)
    #     for row, col in self.markable_tiles:
    #         self.mark_tile(grid_surface, row, col, grid)

    # def calculate_tile_dimensions(self, row, col, border_thickness):
    #     return {
    #         "center_x": self.tile_size * col + self.tile_size / 2,
    #         "center_y": self.tile_size * row + self.tile_size / 2,
    #         "size": self.tile_size - border_thickness
    #     }

    # def create_tile_highlight(self, x, y, size, color, alpha):
    #     highlight = pygame.Surface((size, size), pygame.SRCALPHA)
    #     highlight.fill((*color, int(255 * alpha)))
    #     rect = highlight.get_rect(center=(x, y))
    #     return highlight, rect

    # def calculate_border_sides(self, row, col, grid):
    #     borders = {"top": True, "right": True, "bottom": True, "left": True}
    #     is_adjacent_tile_marked = lambda r, c: (r, c) in self.markable_tiles

    #     if (row, col) in self.markable_tiles:
    #         if row > 0 and is_adjacent_tile_marked(row - 1, col):
    #             borders["top"] = False
    #         if col < len(grid[0]) - 1 and is_adjacent_tile_marked(row, col + 1):
    #             borders["right"] = False
    #         if row < len(grid) - 1 and is_adjacent_tile_marked(row + 1, col):
    #             borders["bottom"] = False
    #         if col > 0 and is_adjacent_tile_marked(row, col - 1):
    #             borders["left"] = False
    #     return borders

    # def mark_tile(self, grid_surface, row, col, grid):
    #     style = {
    #         "border": {"thickness": 1.5, "color": (92, 64, 51), "alpha": 0.8},
    #         "highlight": {"color": (170, 136, 68), "alpha": 0.9}
    #     }

    #     dims = self.calculate_tile_dimensions(row, col, style["border"]["thickness"])
    #     border_sides = self.calculate_border_sides(row, col, grid)

    #     highlight, rect = self.create_tile_highlight(dims["center_x"], dims["center_y"], dims["size"], style["highlight"]["color"], style["highlight"]["alpha"])
    #     grid_surface.blit(highlight, rect.topleft)

    #     self.draw_borders(grid_surface, dims["center_x"], dims["center_y"], dims["size"], style["border"], border_sides)

    # def draw_borders(self, grid_surface, x, y, size, border_style, border_sides):
    #     thickness = int(border_style["thickness"])
    #     if border_sides["top"]:
    #         pygame.draw.line(grid_surface, border_style["color"], (int(x - size / 2), int(y - size / 2)), (int(x + size / 2), int(y - size / 2)), thickness)
