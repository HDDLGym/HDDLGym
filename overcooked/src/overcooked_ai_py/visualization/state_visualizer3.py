    ...
    def _render_hud_data(self, surface, hud_data):
        ...
        for hud_line_num, (key, value) in enumerate(self._sorted_hud_items(hud_data)):
            ...
            if key == 'time left':
                ...
                hud_text_width = text_surface.get_width()
                if len(hud_text) == 1:
                    # Adjust for single-digit text
                    clock_position = (text_surface_position[0] - hud_text_width / 2 + self.CLOCK_IMG.get_width() / 2 - 170, 
                                    text_surface_position[1] - self.CLOCK_IMG.get_height() / 2 + text_height / 2)
                    hud_text = '' 
                else:
                    # Adjust for multi-digit text
                    clock_position = (text_surface_position[0] - hud_text_width / 2 + self.CLOCK_IMG.get_width() / 2 - 145, 
                                    text_surface_position[1] - self.CLOCK_IMG.get_height() / 2 + text_height / 2)
