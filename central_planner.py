...
def centralized_planner(world, main_agent_index=0, all_agents = [], all_policies = [], debug=False, deterministic=False, device = False):
  ...
  while len(agent_reach_action) < len(agents):
    ...
    # get observation one-hot vector and use it to get probability list from RL policy:
    valid_prob_list = []
    observation_one_hot_vector = get_observation_one_hot_vector(world.current_state, all_operators, world.env_dictionary, device=device)
    ...
    for index_agent, ag in enumerate(agents):
      ...
      elif (len(related_oper_indices) == 2 and 'none '+ag.name in chosen_combination):
        #6.26: note that this won't apply for 3+ agents
        # print("CCP 448: none in this collab combination: ",chosen_combination)
      ...
      # 5.2. if agent appears in more than 2 opers: use agent's policy to choose (random.choices with weight is prob from the policy)
        ...
        prob_op_list = []
        # for op_i in related_oper_indices:
        #   op_index = enumerate_list(chosen_combination[op_i], world.env_dictionary['grounded operators list']).index(1)
