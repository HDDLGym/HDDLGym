    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            if self.adv_normalization:
                adv = (adv - adv.mean()) / ((adv.std() + 1e-10))  #sometimes helps
        ...
        for _ in range(self.K_epochs):
            ...
            for i in range(optim_iter_num):
                ...
                entropy = Categorical(probs = prob_a).entropy()
                a_loss = torch.min(surr1, surr2) - self.entropy_coef * entropy
                a_loss_list.append(a_loss.mean().cpu().detach())
                ...
                c_loss_list.append(c_loss.cpu().detach())
